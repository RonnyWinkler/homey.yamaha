...
class yamaha_av{
    ...
    async isAdaptiveDrcEnabled(useBuffer=false){
        if (!useBuffer || this._basicStatus == {}){
            await this.getBasicStatus();
        }
        if (this._basicStatus == {}){
        ...
        return (this._basicStatus.YAMAHA_AV[this._zone].Basic_Status.Sound_Video.Adaptive_DRC == 'Auto');
    }
    ...
    async isPartyModeEnabled(useBuffer=false){
        if (!useBuffer || this._basicStatus == {}){
            await this.getBasicStatus();
        }
        if (this._basicStatus == {}){
        ...
        return (this._basicStatus.YAMAHA_AV[this._zone].Basic_Status.Party_Info == 'On');
    }
    ...
    async adaptiveDrcOn(){
        ...
        let payload = '<YAMAHA_AV cmd="PUT"><'+this._zone+'><Sound_Video><Adaptive_DRC>Auto</Adaptive_DRC></Sound_Video></'+this._zone+'></YAMAHA_AV>';
        let xml = await this._httpPost(payload);
        let json = parser.parse(xml);
        return json;
    }
    async adaptiveDrcOff(){
        if (this._ip == ''){
            throw new Error('IP not set');
        }
    ...
    async setAdaptiveDrc(state){
        if(state){
            return await this.adaptiveDrcOn();
        }
        else{
            return await this.adaptiveDrcOff();
        }
    }
    async enhancerOn(){
        if (this._ip == ''){
    ...
    async partyModeOn(){
        ...
        let payload = '<YAMAHA_AV cmd="PUT"><System><Party_Mode><Mode>On</Mode></Party_Mode></System></YAMAHA_AV>';
        let xml = await this._httpPost(payload);
        let json = parser.parse(xml);
        return json;
    }
    async partyModeOff(){
        ...
        let payload = '<YAMAHA_AV cmd="PUT"><System><Party_Mode><Mode>Off</Mode></Party_Mode></System></YAMAHA_AV>';
        let xml = await this._httpPost(payload);
        let json = parser.parse(xml);
        return json;
    }
    async setPartyMode(state){
        if(state){
            return await this.partyModeOn();
        }
        else{
            return await this.partyModeOff();
        }
    }
    ...
    async setScene(scene){
        ...
        let payload = '<YAMAHA_AV cmd="PUT"><'+this._zone+'><Scene><Scene_Sel>Scene ' +scene+ '</Scene_Sel></Scene></'+this._zone+'></YAMAHA_AV>';
        let xml = await this._httpPost(payload);
        let json = parser.parse(xml);
        return json;
    }
