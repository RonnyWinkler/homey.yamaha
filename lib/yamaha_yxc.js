...
class YamahaYXC {
	...
	//-------------- Set UDP port for update unicast UDP messages ---------------------------
	setUdpPort(port){
		this._udpPort = port;
	}

	//-------------- general Communication ---------------------------
	async SendReqToDevice(cmd, method='GET', body) {
        return new Promise((resolve, reject) => {
			let ip = this.ip;
			if (this.port) ip = ip + ':' + this.port;
			let url = 'http://' + ip + '/YamahaExtendedControl/v1' + cmd;
			const options = {
				method: method,
				// body,
				uri: url,
				headers: {
					...
					// request change information via UDP unicast message:
					// 'X-AppName': 'MusicCast/1.0',
					// 'X-AppPort': '41100'
				}
			};
			if (this._udpPort){
				options.headers['X-AppName'] = 'MusicCast/1.0';
				options.headers['X-AppPort'] = this._udpPort;
			}
			if (method == 'POST'){
            ...
            req.on('error', (error) => reject(new Error('Failed to '+method+' to url:' + url + error)));
			if (method == 'POST'){
	            req.write(body);
			}
            req.end();
	...
	async setSurroundDecoder(type, zone) {
		//check for correct input in calling program
		try {
			const command = '/' + this.getZone(zone) + '/setSurroundDecoderType?type=' + type;
			const result = await this.SendGetToDevice(command);
			return result;
		} catch (error) {
			throw error;
	...
	async setScene(scene, zone) {
		try {
			const command = '/' + this.getZone(zone) + '/recallScene?num=' + scene;
			const result = await this.SendGetToDevice(command);
			return result;
		} catch (error) {
			throw error;
	...
	async setTunerPreset(item, band, zone) {
		try {
			const command = '/tuner/recallPreset?zone='+ this.getZone(zone) +'&band='+band+'&num='+item;
			const result = await this.SendGetToDevice(command);
			return result;
		} catch (error) {
			throw error;
	...
	async sendIrCode(code) {
		try {
			const command = '/system/sendIrCode?code='+code;
			const result = await this.SendGetToDevice(command);
			return result;
		} catch (error) {
			throw error;
