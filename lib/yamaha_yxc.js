...
class YamahaYXC {
        ...
        return new Promise((resolve, reject) => {
            ...
            const req = http.request(url, options, res => {
                if (res.statusCode !== 200) {
                    ...
                    return reject( new Error('Failed to '+method+' to url:' + url +' status code: '+res.statusCode));
                }
	...
	// 	}
	// }
	// async setLinkAudioQuality(mode) {
	// 	try {
	// 		const command = '/' + this.getZone(zone) + 'setLinkAudioQuality?mode=' + mode;
	// 		const result = await this.SendGetToDevice(command);
	// 		return Promise.resolve(result);
	// 	} catch (error) {
	// 		return Promise.reject(error);
